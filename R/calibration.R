...
#' @param nche A scalar representing the equivalent number of patients borrowed from historical study
...
#' @param datamat A matrix with dimension nsim * 2 containing the pre-simulated data for the study treatment (1st column) and control  (1st column) groups, respectively. Default is NULL, and binomial random Monte Carlo samples will be generated in the function.
#' @param w0 A scale prior power parameters w. If not specified (default), w_d is calculated by the specified method for dynamic borrowing.
#' @param nsim A scalar. Number of replications to calculate power
#' @param seed A scalar. seed for simulations
#'
#' @return The scalar threshold for statistical significance that can control the type I error (1-sided)
#' @examples
#'
