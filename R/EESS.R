...
#' @param nche A scalar. Equivalent number of patients borrowed from historical study
...
#' @param delta_threshold A scalar. Borrow when abs(pc_hat (current study) - pch) <= delta_threshold
...
#' @examples
#'
#' EESS(pc=0.3,nc=40,pch=0.3,nche=40,nch=180, a0c=0.001,b0c=0.001,delta_threshold=0.1,method="Empirical Bayes", theta=0.5, eta=1)
#'
#' @export
#'
EESS = function(pc,
  ...
  w = rep(NA,nc+1)
  for(i in 1:(nc+1)){
    w[i] = borrow.wt(Yc=i-1, nc=nc, Ych=nch*pch, nch=nch, nche=nche,
                     a0c=a0c, b0c=b0c, delta_threshold=delta_threshold, method=method,theta=theta,eta=eta)$w
  }
