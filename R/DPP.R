...
#' @param pc.calib A scalar. Response rate for control arm in current study for calibration. Usually, pc.calib = pch.
#' @param nch A scalar. Total number of patients in historical control
#' @param nc.range A vector with length = 2. Search range for nc. Default is NULL, and the range will be automatically determined.
#' @param r A scalar. Randomization ratio for current study. r=1 means 1:1 and r=2 means 2:1.
#' @param q A scalar. Specification of n_che in terms of times of nc; i.e. n_che = q*nc. Usually, q >= 1 and q <= nch/n_che.
...
#'  \item delta.bound  significance boundary of delta between the study experimental group and study control group
#'  \item mean.PMD mean of posterior mean difference over nsim estimates
#'  \item sd.PMD standard deviation of posterior mean difference over nsim estimates
#'  \item mean_pc_hca a vector of nsim length. Storing the posterior means of pc based on hybrid control for nsim replications.
#'  \item mean_pc_c a vector of nsim length. Storing the posterior means of pc based on study  control for nsim replications.
#'  }
#' @examples
#'
...
DPP = function(pt, pc, pch, pc.calib, nch, nc.range=NULL,
  ...
  for (i in 1:L){
    ...
    if (pow >= power){
      ...
      if (i == L){ans = "No solution found in the provided range of nc"}
    }
  }
  return(ans)
}
